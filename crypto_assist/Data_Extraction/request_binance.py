...
def chart_data():
    ...
    def days_to_fetch(start=today, final=auto_start_date):
        if start == final:
            return start
        else:
            return 0

    def date_check(days_to_fetch=days_to_fetch(), start=today):
        if days_to_fetch == start:
            print("✅ Bitcoin chart dataframe is up to date.")
            return
        else:
            print("Bitcoin data needs to be updated")
            #===================SETTINGS=======================#
            ...
            # Export ======================================================================#
            merged_df.to_csv((os.path.join(route_path, 'raw_data','BTCUSDT_daily.csv')), mode='a', header=False, index=True)
            print("✅ Bitcoin chart dataframe is updated.")
